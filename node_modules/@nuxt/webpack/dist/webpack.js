/*!
 * @nuxt/webpack v2.4.5 (c) 2016-2019
...
PerfLoader.warmup = threadLoader.warmup;
...
class PostcssConfig {
  ...
  config() {
    ...
    config = this.normalize(cloneDeep_1(this.postcss));
...
class StyleLoader {
  ...
  styleResource(ext) {
    ...
    if (extResource) {
      const patterns = utils.wrapArray(extResource).map(p => path.resolve(this.rootDir, p));
...
class WebpackBaseConfig {
  ...
  getBabelOptions() {
    const options = clone_1(this.options.build.babel);
  ...
  output() {
    return {
      ...
      publicPath: utils.isUrl(this.options.build.publicPath)
        ? this.options.build.publicPath
        : utils.urlJoin(this.options.router.base, this.options.build.publicPath)
    }
  }

  optimization() {
    const optimization = cloneDeep_1(this.options.build.optimization);
  ...
  config() {
    ...
    const extendedConfig = this.extendConfig(cloneDeep_1(config));
